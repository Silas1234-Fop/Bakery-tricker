...
class PackagingScreen(Screen):
    ...
    def _build_ui(self):
        ...
        dark_bg(root)
        root.add_widget(header_bar("📦 Packaging Entry",
        ...
        body.add_widget(field_label("Total Packed (units)"))
        self.packed_input = text_field("e.g. 300", numeric=True)
        body.add_widget(self.packed_input)
    ...
    def save_record(self, *args):
        ...
        packed   = self.packed_input.text.strip()
        rec_date = self.date_input.text.strip()

        if not all([dept, product, packed, rec_date]):
            alert_popup("Please fill in all fields."); return
        try:
            packed_q = float(packed)
        except ValueError:
            alert_popup("Total packed must be a number."); return
        if packed_q <= 0:
            alert_popup("Quantity must be greater than zero."); return

        db.add_packaging(dept, product, packed_q, rec_date)
        alert_popup(f"✅ Saved!\n{product} – Packed: {packed_q:.0f} units")
        self.packed_input.text = ""
        self._refresh_records()
    ...
    def _refresh_records(self):
        ...
        records = db.get_packaging(department=dept)[:10]
        for r in records:
    ...
    def _record_row(self, r):
        ...
        info.add_widget(Label(
            text=f"{r['department']} · {r['product']}",
            font_size=dp(12), color=(1, 1, 1, 1), halign="left",
            text_size=(None, None)
        ))
        info.add_widget(Label(
            text=f"Packed: {r['total_packed']}  {r['rec_date']}",
